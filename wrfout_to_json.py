...
wrfout = '/home/ec2-user/cnaps2json/20181106.nc'
outdir = '/home/ec2-user/cnaps2json/json/'
...
[d_lon,d_lat] = np.meshgrid(np.arange(min(lon[0,:])-1e-2, max(lon[0,:])+1e-2, dx),\
                           np.arange(min(lat[:,0])-1e-2, max(lat[:,0])+1e-2, dy))
...
for t in range (0,size(time)):
    ...
    outfile = outdir + str(time[t]) + '_sfc_wind.json'
    
    print(outfile)

    json_out = open(outfile,'w')
    curr_time = str(time[t])
    yyyy = curr_time[0:4]
    mm = curr_time[4:6]
    dd = curr_time[6:8]
    hh = curr_time[8:10]
